...
def load_data(data_file):
    ...
    lines = list(zip(list(data_df['text']), list(data_df['label'])))
    return lines
...
class DataIterator:
    ...
    def convert_single_example(self, example_idx):
        sentence = self.data[example_idx].sentence.strip().split(' ')
...
if __name__ == '__main__':
    ...
    train_iter = DataIterator(config.batch_size,
                              data_file='train.csv', use_bert=config.use_bert,
                              tokenizer=tokenizer, seq_length=config.sequence_length)
    dev_iter = DataIterator(config.batch_size, data_file='dev.csv', use_bert=config.use_bert,
                            seq_length=config.sequence_length, is_test=True, tokenizer=tokenizer)
    for input_ids_list, input_mask_list, segment_ids_list, labels_list, seq_length in tqdm(train_iter):
