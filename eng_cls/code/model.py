...
class BertForCLS(BertPreTrainedModel):
    def __init__(self, config, params):
        ...
        if params.model_type == 'bilstm':
            ...
            from torch_utils.gcn import GAT
            lstm_num = nclass = int(self.params.embed_dense / 2)
            self.lstm = GAT(nfeat=self.params.embed_dense, nhid=lstm_num,
                            nclass=lstm_num, dropout=0.2, alpha=0.01,
                            nheads=3)
        # 全连接分类组件
